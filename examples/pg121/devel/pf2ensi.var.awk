...
function start_bnd() {
    ...
    print "Generating variable file: " var_base_filename > "/dev/stderr";

    print "Alya Ensight Gold --- Scalar per-partial-node variable file" > var_base_filename;
    print "part" > var_base_filename;
    print "1" > var_base_filename;

    if( !do_partial ) {
	print "coordinates" > var_base_filename;
    } else {
	print "coordinates partial" > var_base_filename;
    }
}
...
function start_fix() {
    ...
    print "Alya Ensight Gold --- Vector per-partial-node variable file" > var_base_filename;
    print "part" > var_base_filename;
    print "1" > var_base_filename;
    if( !do_partial ) {
	print "coordinates" > var_base_filename;
...
function end_fix_nopartial_vector() {
    ...
    # system("cat " tmp_pid_file " >> " var_file);
    system("cat " tmp_icomp_file " >> " var_base_filename);
    system("cat " tmp_jcomp_file " >> " var_base_filename);
    system("cat " tmp_kcomp_file " >> " var_base_filename);
...
function start_lds() {
    ...
    # Open variable file for this partial scalar data; single time step
    var_base_filename = var_base_filename;
    print "Generating variable file: " var_base_filename > "/dev/stderr";
...
function end_lds_nopartial_vector() {
    ...
    # close the var_base_filename and temporary files
    close(var_base_filename);
    close(tmp_pid_file);
    close(tmp_icomp_file);
    close(tmp_jcomp_file);
    ...
    # append each node id and vector component to the var_base_filename
    # system("cat " tmp_pid_file " >> " var_base_filename);
    system("cat " tmp_icomp_file " >> " var_base_filename);
    system("cat " tmp_jcomp_file " >> " var_base_filename);
    system("cat " tmp_kcomp_file " >> " var_base_filename);
