...
! MAIN EXECUTION
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    CALL syncwrite_log('> parallel_setup()');   tmp_time = curnt_time

    !-- partition dual elements (eq. primal nodes) across available processes --
    CALL syncwrite_log('> parallel_setup() - partition_dual()')
    CALL partition_dual();   CALL syncwrite_log_time()

    !-- get local node indices of highest order primal elements --
    CALL syncwrite_log('> parallel_setup() - read_prml_elms()');
    CALL read_prml_elms2();   CALL syncwrite_log_time()

    !-- get local connectivity and setup parallel connectivity --
    CALL syncwrite_log('> parallel_setup() - calc_bndry_cobndry()')
    tmp_time_int = curnt_time; CALL calc_bndry_cobndry()
    curnt_time = tmp_time_int;  CALL syncwrite_log_time()

    !-- get parallel connectivity --
    CALL syncwrite_log('> parallel_setup() - get parallel connectivity '// &
      '- get_glb_indx_dual_vlm')
    CALL get_glb_indx_dual_vlm();   CALL syncwrite_log_time()
    CALL syncwrite_log('> parallel_setup() - get parallel connectivity '// &
      '- get_connectivity()')
    CALL get_connectivity();   CALL syncwrite_log_time()
    CALL syncwrite_log('> parallel_setup() - get parallel connectivity '// &
      '- get_glb_indx')
    CALL get_glb_indx();   CALL syncwrite_log_time()

    !-- read primal node locations --
    CALL syncwrite_log('> parallel_setup() - read primal node locations')
    CALL read_nodes_prml();   CALL exchange_prml_nodes()
    CALL syncwrite_log_time()

    !-- log time to complete parallel setup --
    curnt_time = tmp_time;   CALL syncwrite_log_time()
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    IF (rank == root) THEN
      !-- Open primary element file and check for errors --
      fname = trim(mesh_prefix) // ".ele"
      OPEN(mesh_unit,FILE=fname,STATUS='OLD',ACTION='READ',IOSTAT=ier)
      IF (ier /= 0) THEN
        WRITE(log_unit,'(A,A,A,I5)')'***Error: failed to open ',trim(fname),&
          ': errcode = ',ier
        err = .TRUE.
      ELSE
        ...
        IF (dim_cmplx<1) THEN
          WRITE(log_unit,'(A,A,A,I5)')'***Error: in element file, ',&
            trim(fname),': dimension of simplicial complex less than one: ',&
            dim_cmplx
          err = .TRUE.
        END IF
      END IF
      ...
      ELSE
        ...
        !-- first node to get index offset
        READ(mesh_unit,*) junk
        indx_offset(1) = 1-junk
        ...
        IF (glb_num_elm(1) < num_procs) THEN
          WRITE(log_unit,'(A,I5,A,A,I5)')'***Error: number of available '//&
            'processes, ', num_procs, ', greater than the number of dual '//&
            'elements (eq. primary nodes), ',glb_num_elm(1)
          err = .TRUE.
        END IF
      END IF
      ...
      !-- pack MPI buffer --
      int_buffer(1) = k
      int_buffer(2) = dim_embbd
      int_buffer(3) = glb_num_elm(1)
      int_buffer(4) = glb_num_elm(k)
      int_buffer(5) = indx_offset(1)
      int_buffer(6) = indx_offset(k)
    END IF
    ...
    IF (rank /= root) THEN
      k               = int_buffer(1)
      dim_embbd       = int_buffer(2)
      glb_num_elm(1)  = int_buffer(3)
      glb_num_elm(k)  = int_buffer(4)
      indx_offset(1)  = int_buffer(5)
      indx_offset(k)  = int_buffer(6)
      dim_cmplx       = k - 1
    END IF
    ...
    !   ranges --
    extra_pelm = mod(glb_num_elm(1),num_procs)
    IF (rank < extra_pelm) THEN
      num_elm(1) = num_elm(1) + 1
      glb_offset = rank*num_elm(1)
    ELSE
      glb_offset = extra_pelm + rank*num_elm(1)
    END IF

    !-- gather the number of elements per_node --
    IF (ALLOCATED(num_pelm_pp)) DEALLOCATE(num_pelm_pp)
    ALLOCATE(num_pelm_pp(num_procs))
    CALL MPI_ALLGATHER(num_elm(1),1,MPI_INTEGER,num_pelm_pp,1,MPI_INTEGER,&
      MPI_COMM_WORLD,ier)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    DO j = 1,lcl_complex(2)%num_recv
      IF (j==1 .OR. lcl_complex(2)%recv_indx(j,1) /= &
          lcl_complex(2)%recv_indx(max(1,j-1),1)) &
        num_adj_proc = num_adj_proc + 1
    END DO
    ALLOCATE(adj_proc(num_adj_proc))

    !-- allocate MPI arrays --
    ALLOCATE(&
      num_send(dim_cmplx,num_adj_proc),&
      num_recv(dim_cmplx,num_adj_proc),&
      req(dim_cmplx*num_adj_proc),&
      status(MPI_STATUS_SIZE,dim_cmplx*num_adj_proc))
    ...
    DO j = 1,lcl_complex(2)%num_recv
      IF (j==1 .OR. lcl_complex(2)%recv_indx(j,1) /= &
          lcl_complex(2)%recv_indx(max(1,j-1),1)) THEN
        ptr = ptr + 1
        adj_proc(ptr) = lcl_complex(2)%recv_indx(j,1)
        num_recv(1,ptr) = 1
      ELSE
        num_recv(1,ptr) = num_recv(1,ptr) + 1
      END IF
    END DO
    ...
    DO k=3,dim_cmplx+1
      ...
      DO j = 1,lcl_complex(k)%num_recv
        IF (j==1 .OR. lcl_complex(k)%recv_indx(j,1) /= &
          ...
          num_recv(km,ptr) = 1
        ELSE
          num_recv(km,ptr) = num_recv(km,ptr) + 1
        END IF
      END DO
      ptr2 = ptr2 + lcl_complex(k)%num_recv*(k-1)
    END DO
    ...
    DO j = 1,num_adj_proc
      ptr = (j-1)*dim_cmplx;  ptr2 = ptr + dim_cmplx
    ...
    DO k=2,dim_cmplx+1
      ...
      DO j = 1,num_adj_proc
        ...
        CALL MPI_ISEND(sbuffer(ptr2:ptr3-1),km*num_recv(km,j),MPI_INTEGER,&
          adj_proc(j),k,MPI_COMM_WORLD,junk,ier)
    ...
    DO k=2,dim_cmplx+1
      km = k-1
      ptr2 = 1 !-- lcl_complex(k)%recv_indx pointer --
      DO j = 1,num_adj_proc
        IF (num_send(km,j) == 0) THEN
          req((k-2)*num_adj_proc+j) = MPI_REQUEST_NULL
          CYCLE
        END IF
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    ALLOCATE(lcl_complex(1)%glb_indx(num_elm(1)))
    lcl_complex(1)%glb_indx = lcl_complex(1)%node_indx(:,1)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    DO k = 2,dim_cmplx
      ...
      CALL read_glb_indx2(k)
      CALL exchange_glb_indx(k)
      !CALL read_glb_indx2(k)
    END DO
