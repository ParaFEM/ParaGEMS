...
!===============================================================================
  ...
  !-- initialise solution --
  CALL start_petsc();   CALL initialise_darcy2()

  !-- get necessary matrices and vectors (A x = b) --
  CALL get_LHS_darcy2();   CALL get_RHS_darcy2()
  ...
  DO crck_id=1,max_crcks
    ...
    ELSEIF (crck_type==3) THEN
      DO i = 1, crcks_pstep
        CALL identify_crack6(crck_id,exit_cond)
      END DO
    ELSEIF (crck_type==4) THEN
      CALL identify_crack7(crck_id,exit_cond)
    END IF
    IF (exit_cond) EXIT
